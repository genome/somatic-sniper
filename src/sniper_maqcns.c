...
KHASH_SET_INIT_STR(library)
#define MAX_WINDOW 55 
...
{
	sniper_maqindel_opt_t *mi = (sniper_maqindel_opt_t*)calloc(1, sizeof(sniper_maqindel_opt_t));
	mi->q_indel = 40;
	mi->r_indel = 0.00015;
	//
	mi->mm_penalty = 3;
...
void sniper_maqindel_ret_destroy(sniper_maqindel_ret_t *mir)
{
	if (mir == 0) return;
								 ...
								 int _n_types, int *_types, void * data)
{
    ...
    bam_header_t *h = (bam_header_t*)data;
	sniper_maqindel_ret_t *ret = 0;
	...
	{ // the core part
		...
		{ // get final result
			...
			free(sum);
			// write ret
			ret = (sniper_maqindel_ret_t*)calloc(1, sizeof(sniper_maqindel_ret_t));
			...
			// write count
          khash_t(library) *set1;
          set1 = kh_init(library);
         khash_t(library) *set2;
          set2 = kh_init(library);
			...
			if (p->indel == ret->indel1){
                    ++ret->cnt1;
                    if(rg) {
                      khint_t key = kh_get(library, set1,rg); 
                      if(key == kh_end(set1)) {
                         key = kh_put(library, set1, rg, &khash_ret);
                      }
                    }
                }
                else if (p->indel == ret->indel2){
                    ++ret->cnt2;
                    if(rg) {
                      khint_t key = kh_get(library, set2,rg); 
                      if(key == kh_end(set2)) {
                          key = kh_put(library, set2, rg, &khash_ret);
                    }
                    }
                }
				else ++ret->cnt_anti;
			}
         ret->libs1=kh_size(set1);
          ret->libs2=kh_size(set2);
         kh_destroy(library, set1);
          kh_destroy(library, set2);
			// write gl[]
			ret->gl[0] = ret->gl[1] = 0;
			for (j = 0; j < n; ++j) {
...
{
    int i, j, n_types=2, *types, left, right, max_ins;
    sniper_maqindel_ret_t *ret = 0;
    types = (int*)calloc(n_types, sizeof(int));
    types[0]=tumor_indel->indel1;
    types[1]=tumor_indel->indel2;
    if(tumor_indel->indel1 > tumor_indel->indel2) {
        max_ins=tumor_indel->indel1;
    }else{
    ...
    {
        char *ref2;
        int k, l, *score, *pscore;
        ref2 = (char*)calloc(right - left + max_ins + 2, 1);
        ...
        for (i = 0; i < n_types; ++i) {
            ...
            for (j = 0; j < n; ++j) {
                ...
                cigar = bam1_cigar(p->b);
                bam_segreg(pos, c, cigar, &seg);
                uint32_t mapq = c->qual;
                for (ps = s = 0, l = seg.qbeg; c->pos + l < right && l < seg.qend; ++l) {
                    int cq = bam1_seqi(bam1_seq(p->b), l), ct;
                    ct = c->pos + l >= left? ref2[c->pos + l - left] : 15; // "<" will happen if reads are too long
                    if (cq < 15 && ct < 15) {
                        s += cq == ct? 1 : -mi->mm_penalty;
                        uint32_t q = (uint32_t) bam1_qual(p->b)[l];
                        if (cq != ct) ps += q > mapq ? mapq : q;
                    }
                }
                score[i*n + j] = s; pscore[i*n + j] = ps;
                if (types[i] != 0) { // then try the other way to calculate the score
        ...
        { // get final result
            ...
            int max1_i=0, max2_i=1;
            // write ret
            ret = (sniper_maqindel_ret_t*)calloc(1, sizeof(sniper_maqindel_ret_t));
            ret->indel1 = types[max1_i]; ret->indel2 = types[max2_i];
            // write indel sequence
            ret->s[0] = strdup(tumor_indel->s[max1_i]);
            ret->s[1] = strdup(tumor_indel->s[max2_i]);        
            for (j = 0; j < n; ++j) {
